...
def validate_file_sha256sum(entry, errors):
    with open(entry.fname, 'rb', buffering=0) as f:
        ...
        if hash != entry.sha256:
            errors.add(f'{entry.fname}: expected: {entry.sha256}, got: {hash}')


def validate_mtree_entry(entry) -> set[str]:
    try:
    ...
    if st.st_uid != entry.uid:
        errors.add(f'{entry.fname}: got uid {st.st_uid}, expected: {entry.uid}')
    if st.st_gid != entry.gid:
        errors.add(f'{entry.fname}: got gid {st.st_gid}, expected: {entry.gid}')

    match entry.type:
        case 'dir':
            if not S_ISDIR(st.st_mode):
                errors.add(f'{entry.fname}: incorrect file type.')
        case 'file':
    ...
    if oct(S_IMODE(st.st_mode))[2:] != entry.mode:
        errors.add(f'{entry.fname}: got mode {oct(S_IMODE(st.st_mode))}, expected: {entry.mode}')

    return errors


def process_chunk(chunk) -> set[str]:
    errors = set()
    for line in chunk:
